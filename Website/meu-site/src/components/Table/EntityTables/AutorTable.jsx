...
import EditAutorCard from '../../Form/EntityForms/EditAutorCard';
  ...
  return (
    <div>
      ...
      {viewMode === 'table' ? (
        ...
        <div className="records-card-viewi">
          {filteredAutores.map(autor => (
            <div key={autor.id} className="record-cardi">
              <div className="book-photo-smalli">{autor.foto ? <img src={autor.foto} alt={autor.nome} /> : 'sem imagem'}</div>
              <div className="card-contenti">
                <div className="card-titlei">{autor.nome}</div>
                <div className="card-metai">{autor.email || ''}</div>
